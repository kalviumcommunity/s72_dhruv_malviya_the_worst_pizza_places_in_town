...
const PizzaPlaceCard = ({ pizzaPlace, onDelete }) => {
  ...
  const handleDelete = async () => {
    if (!isAuthenticated) {
      toast({
        title: 'Authentication required',
        description: 'Please log in to delete',
        status: 'warning',
        duration: 3000,
        isClosable: true,
      });
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`/api/pizza-places/${_id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

      toast({
        title: 'Pizza place deleted',
        status: 'success',
        duration: 2000,
        isClosable: true,
      });

      onDelete(_id); // Call the onDelete function passed as a prop
    } catch (error) {
      toast({
        title: 'Error',
        description: error.response?.data?.message || 'Failed to delete pizza place',
        status: 'error',
        duration: 3000,
        isClosable: true,
      });
